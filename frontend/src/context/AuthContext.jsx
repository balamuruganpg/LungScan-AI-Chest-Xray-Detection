...
export const AuthProvider = ({ children }) => {
  ...
  const getStoredDoctors = () => {
    try {
      ...
      if (d) {
        return JSON.parse(d).filter((doc) => doc.email !== 'balamuruganpg@outlook.com');
      }
      return [];
    } catch {
      return [];
  ...
  const login = async (email, password) => {
    ...
    if (normalizedEmail === 'balamuruganpg@outlook.com' || normalizedEmail.includes('admin')) {
      if (supabase) {
        try {
